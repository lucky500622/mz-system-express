...
export const productPageActionInfo: Controller<void> = async (req, res, next) => {
  try {
    const { offset = 0, limit = 10, m_id, product_m_id, product_name, action_type, user_name, start_time, end_time } = req.query
    const pageOffset = Number(offset)
    const pageLimit = Number(limit)
    const actionInfo = await productPageActionInfoModel(pageOffset, pageLimit, Number(m_id), Number(product_m_id), String(product_name || ''), Number(action_type), String(user_name || ''), String(start_time || ''), String(end_time || ''))
    res.json({
      code: 200,
      message: '产品操作信息获取成功',
      data: {
...
// 获取仓库下全部产品名
export const getWarehouseProductName: Controller<void> = async (req, res, next) => {
  try {
    const { text, limit = 10 } = req.query
    const name = await warehouseProductNameModel(String(text || ''), Number(res.locals.warehouseInfo.m_id), Number(limit))
    res.json({
      code: 200,
      message: '仓库产品名获取成功',
      data: {
