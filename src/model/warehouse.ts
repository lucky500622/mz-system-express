...
// 分页获取仓库信息
export const warehousePageInfoModel = async (offset: number, limit: number, connection?: any) => {
...
// 分页获取仓库操作信息
export const warehousePageActionInfoModel = async (offset: number, limit: number, connection?: any) => {
  const exec = (connection || pool) as typeof pool
...
// 获取仓库信息
export const warehouseInfoModel = async (m_id: string, connection?: any) => {
  const exec = (connection || pool) as typeof pool
...
// 新增仓库
export const addWarehouseModel = async (warehouse_id: string, warehouse_name: string, user_id: string, warehouse_type?: string, warehouse_description?: string, connection?: any) => {
  const exec = (connection || pool) as typeof pool
  // 拼接字段数组与值数组
...
// 新增仓库操作信息
export const addWareActionInfoModel = async (issue_id: string, warehouse_id: string, action_type: number, warehouse_rename?: string, connection?: any) => {
  const exec = (connection || pool) as typeof pool
  // 拼接字段数组与值数组
