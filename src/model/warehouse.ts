...
export const warehousePageInfoModel = async (offset: number, limit: number, m_id?: number, warehouse_name?: string, warehouse_type?: string, user_name?: string, connection?: any): Promise<RowDataPacket[]> => {
  ...
  const sql = `SELECT t_warehouse.m_id, warehouse_name_ed, warehouse_name, warehouse_type, user_name, warehouse_description, IF(MAX(product_list_num) = 0,0,1) AS exists_list_product, warehouse_create_time FROM t_warehouse INNER JOIN t_user ON t_warehouse.warehouse_creater_id = t_user.user_id LEFT OUTER JOIN t_product ON warehouse_id = product_belong_id AND t_product.is_delete = 0 WHERE t_warehouse.is_delete = 0 ${where}${fieldArr.join(' AND ')} GROUP BY t_warehouse.m_id ORDER BY m_id DESC LIMIT ?, ?`
...
export const warehouseInfoModel = async (m_id: number, connection?: any): Promise<RowDataPacket> => {
  const exec = (connection || pool) as typeof pool
  const sql = 'SELECT t_warehouse.m_id, warehouse_id, warehouse_name, warehouse_create_time, warehouse_name_ed, COUNT(t_product.product_num) AS product_num, IF(MAX(product_list_num) = 0,0,1) AS exists_list_product FROM t_warehouse LEFT OUTER JOIN t_product ON t_warehouse.warehouse_id = t_product.product_belong_id AND t_product.is_delete = 0 WHERE t_warehouse.m_id = ? AND t_warehouse.is_delete = 0 GROUP BY t_warehouse.m_id'
